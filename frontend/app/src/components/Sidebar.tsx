...
interface SidebarProps {
  onSelectPriceId: (priceId: string, description: string, gsymbol: string, cms: string) => void
}
...
export default function Sidebar({ onSelectPriceId }: SidebarProps) {
  ...
  const handleItemClick = (priceId: string, description: string, gSymbol: string, csmSymbol: string) => {
    if (gSymbol) {
      onSelectPriceId(priceId, description, gSymbol, csmSymbol)
    }
    if (csmSymbol) {
      onSelectPriceId(priceId, description, gSymbol, csmSymbol)
    }
        ...
        {['Equity', 'Crypto', 'FX', 'Metal', 'Rates', 'Commodities'].map((type) => (
          <div className='mb-4 pl-2 pr-2' key={type}>
            <button
              ...
              {type}
              <span className='ml-2'>{expanded[type as AssetType] ? '-' : '+'}</span>
            </button>
                ...
                {(type === 'Commodities'
                  ...
                  (type === 'Commodities'
                    ...
                    <li key={index} className={`py-2 ${index > 0 ? 'border-t border-gray-500' : ''}`}>
                      {selected == asset.priceId ? (
                        <a
                          href='#'
                          onClick={() =>
                            handleItemClick(asset.priceId, asset.description, asset.generic_symbol, asset.cms_symbol)
                          }
