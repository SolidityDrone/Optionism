...
export default function Dashboard() {
  ...
  const { data: hash, error: claimerr, isPending, writeContract } = useWriteContract()
              ...
              <tbody>
                {wrote.length > 0 ? (
                  wrote.map((option) => {
                    ...
                    const priceColor = option.isCall
                      ? parseFloat(currentPrice) > adjustedStrikePrice
                        ? 'text-green-500'
                        : 'text-red-500'
                      : parseFloat(currentPrice) < adjustedStrikePrice
                        ? 'text-green-500'
                    ...
                    return (
                      <tr key={option.id} className='border-b bg-gray-900 hover:bg-gray-600'>
                        <td className='w-[80px]  text-center flex items-center justify-center h-[44px]'>
                          <div
                            className={`w-4 h-4 rounded-full ${option.isCall ? 'bg-green-500' : 'bg-red-500'}`}></div>
                        </td>
                        <td className='w-[134px] text-center'>{getSymbolFromPriceId(option.priceId)}</td>
                        <td className={`w-[134px] text-center ${priceColor}`}>{currentPrice}$</td>
                        <td className='w-[134px] text-center'>{adjustedStrikePrice.toFixed(6)}$</td>
                        ...
                        <td className='w-[134px] text-center'>
                          {parseFloat(option.responseValue!) * Math.pow(10, priceData.expo)
                            ? parseFloat(option.responseValue!) * Math.pow(10, priceData.expo)
                            : '-'}
                        </td>
                      </tr>
                  ...
                  bought.map((option) => {
                    ...
                    return (
                      <tr key={option.priceId} className='border-b bg-gray-900 hover:bg-gray-600'>
                        ...
                        <td className='w-[80px] text-center'>{option.units}</td>
                        <td className='w-[134px] text-center'>
                          {parseFloat(option.responseValue!) * Math.pow(10, priceData.expo)
                            ? parseFloat(option.responseValue!) * Math.pow(10, priceData.expo)
                            : '-'}
