...
contract Optionism is IOptionism, ERC1155 {
    ...
    modifier onlyGelato() {
        if (msg.sender != gelatoAddress) revert NotGelato();
        _;
    }
    ...
    function buyOption(uint id, uint amountOfShares) public {
    ...
    function claimOptionWin(uint256 id) public {
        ...
        isCall
            ? finalGainPerShare = (results[id] - option.strikePrice)
            : (option.strikePrice - results[id]);
        finalGainPerShare > option.maxiumPayoutPerShare
            ? finalGainPerShare = option.maxiumPayoutPerShare
            : finalGainPerShare;
    ...
    function getArrayChunk(
        ...
        view
        returns (
            uint[] memory optionIDs,
            uint[] memory expiries,
        ...
        )
    {
        ...
        for (uint i = startIndex; i < endIndex; i++) {
            if (options[optionsArray[i]].isActive) {
                activeCount++;
            }
        }

        optionIDs = new uint[](activeCount);
        expiries = new uint[](activeCount);
        priceIds = new bytes32[](activeCount);
    ...
    function gelatoCallBack(
        ...
        for (uint i = 0; i < optionIds.length; i++) {
            bool toPay;
            bool isCall = options[optionIds[i]].isCallOption;
            Option memory op = options[optionIds[i]];

            try pyth.getPrice(op.assetID) returns (
            ...
            if (isCall) {
                toPay = uint64(price.price) > op.strikePrice;
            } else {
                toPay = uint64(price.price) < op.strikePrice;
            }
            options[optionIds[i]].hasToPay = toPay;
            options[optionIds[i]].isActive = false;
