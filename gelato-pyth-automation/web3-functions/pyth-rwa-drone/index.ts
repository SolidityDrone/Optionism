...
const storageAwaitingValue = await storage.get("storageAwaiting");
...
const chunk = await optionismContract.getArrayChunk(0, 2);
...
await storage.set("storageAwaiting", newStorageAwaitingValue);
...
console.log("Filtered Option IDs for processing:", validOptionIdsArray);
  ...
  const priceFeeds = (await connection.getLatestPriceFeeds(chunkPriceIdsArray)) as any[];
...
const currentPrice: IPRICE = {
  price: +BigNumber.from(priceFeeds[0].price.price),
  timestamp: +BigNumber.from(priceFeeds[0].price.publishTime),
};

// Encode optionId, priceId, and priceResult
return utils.defaultAbiCoder.encode(
  ["uint256", "bytes32", "uint256"],  // uint256 for priceResult
  [optionId, validPriceIdsArray[index], currentPrice.price]
);
});

// Logging for debugging
console.log("Encoded Items Count:", encodedItems.length);
...
const callData = iface.encodeFunctionData("gelatoCallBack", [encodedItems]);
